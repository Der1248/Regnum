...
zcg.add_craft = function(input, output, groups)
    local str = minetest.registered_items[output].description
    if string.find(str, "\n") then
        if string.find(str, "lv.0") then
        else
            return
        end
    end
...
zcg.formspec = function(pn)
	...
	local formspec = "size[16,12.5]"
	.. "button[0,0;2,.5;inven;Back]"
	.. "button[0,1;2,.5;main;Main]"
    .. "button[0,2;2,.5;recraft;Regnum]"
    .."background[16,12.5;1,1;gui_formbg.png;true]"
    .."listcolors[#00000069;#5A5A5A;#141318;#30434C;#FFF]"
    .."bgcolor[#080808BB;true]"
	if zcg.users[pn].history.index > 1 then
		formspec = formspec .. "image_button[0,3;1,1;zcg_previous.png;zcg_previous;;false;false;zcg_previous_press.png]"
	else
		formspec = formspec .. "image[0,3;1,1;zcg_previous_inactive.png]"
	end
	if zcg.users[pn].history.index < #zcg.users[pn].history.list then
		formspec = formspec .. "image_button[1,3;1,1;zcg_next.png;zcg_next;;false;false;zcg_next_press.png]"
	else
		formspec = formspec .. "image[1,3;1,1;zcg_next_inactive.png]"
	end
	-- Show craft recipe
	if current_item ~= "" then
		if zcg.crafts[current_item] then
			...
			if alt > 1 then
				formspec = formspec .. "button[9,0;1,1;zcg_alt:"..(alt-1)..";^]"
			end
			if alt < #zcg.crafts[current_item] then
				formspec = formspec .. "button[9,2;1,1;zcg_alt:"..(alt+1)..";v]"
			end
			...
			if c then
				...
				if c.type == "normal" or c.type == "cooking" then
					formspec = formspec .. "image[8,2;1,1;zcg_method_"..c.type..".png]"
				else -- we don't have an image for other types of crafting
					formspec = formspec .. "label[0,2;Method: "..c.type.."]"
				end
				formspec = formspec .. "image[8,1;1,1;zcg_craft_arrow.png]"
				formspec = formspec .. "item_image_button[9,1;1,1;"..zcg.users[pn].current_item..";;]"
			end
		end
	end
	
	-- Node list
	local npp = 16*6 -- nodes per page
	...
	for _, name in ipairs(zcg.itemlist) do
		if s < page*npp then s = s+1 else
			...
			formspec = formspec .. "item_image_button["..(i%16)..","..(math.floor(i/16)+5.5)..";1,1;"..name..";zcg:"..name..";]"
			i = i+1
		end
	end
	if page > 0 then
		formspec = formspec .. "button[0,12;1,.5;zcg_page:"..(page-1)..";<<]"
	end
	if i >= npp then
		formspec = formspec .. "button[1,12;1,.5;zcg_page:"..(page+1)..";>>]"
	end
	formspec = formspec .. "label[2,11.85;Page "..(page+1).."/"..(math.floor(#zcg.itemlist/npp+1)).."]" -- The Y is approximatively the good one to have it centered vertically...
	return formspec
end
