...
local function validate_armor_inventory(player)
	...
	for i = 1, 6 do
		...
		if stack:get_count() > 0 then
			...
			--change start
			if not element then
				if minetest.get_item_group(item, "heart") == 1 then
					element = "heart"
				end
			end
			--change end
		...
		allow_put = function(inv, listname, index, put_stack, player)
			...
			if listname == "armor" then
		        ...
		        elseif index == 5 and put_stack:get_definition().groups.armor_shield then
			      return 1
				elseif index == 6 and put_stack:get_definition().groups.heart then
			      return 1
			    else
			      return 0
			    end
            elseif listname == "arm2" then
				if string.find(put_stack:get_name(), "tutorial:protection_schluessel") then
                	return 1
				else
					return 0
				end
...
minetest.register_on_player_hpchange(function(player, hp_change, reason)
	...
	if hp + hp_change < properties.hp_max then
		...
		--change start
		local level = armor.def[name].level
		local decimal_part = math.abs(hp_change*(100-level)*0.01)
		hp_change = hp_change*(100-level)*0.01
		if math.random() < decimal_part then
			hp_change = math.floor(hp_change)
		else
			hp_change = math.floor(hp_change+1)
		end
		--change end
...
minetest.register_globalstep(function(dtime)
	...
	if armor.config.feather_fall == true then
		for _,player in pairs(minetest.get_connected_players()) do
			local name = player:get_player_name()
			if armor.def[name].feather == 1 then	--changed
