...
armor.set_player_armor = function(self, player)
	...
	for i, stack in pairs(list) do
		...
		if stack:get_count() == 1 and minetest.get_item_group(item, "heart") ~= 1 then
		--change end
			...
			if not transparent_armor and coun == 0 then
	...
	elseif use_pova_mod then
		...
		if player_physics_locked == nil or player_physics_locked == 0 then
			...
			physics.speed = speed_count
			physics.jump = jump_count
