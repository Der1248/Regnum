...
local function show_item(def)
	local show = true
	local str = def.description
    if string.find(str, "-Lv.") then
        if string.find(str, "Lv.0") then
        else
            show = false
        end
    end
	return def.groups.not_in_craft_guide ~= 1 and def.description ~= "" and show
end
...
local function item_button_fs(fs, x, y, item, element_name, groups, width)
	table.insert(fs, ("item_image_button[%s,%s;%s,%s;%s;%s;%s]")
		:format(x, y, width, width, item, element_name, groups and "\n"..esc(S("G")) or ""))
...
local function recipe_fs(fs, data)
	...
	for i, item in pairs(recipe.items) do
		...
		local button_width = 1.05
		local button_offset = 1
		if width == 5 or rows == 5 then
			button_width = 0.7
			button_offset = 0.575 
		elseif width == 4 or rows == 4 then
			button_width = 0.8
			button_offset = 0.7333
		end
		local base_x = 3 - width*button_offset
		...
		item_button_fs(fs, base_x + x*button_offset, base_y + y*button_offset, item, elem_name, groups, button_width)
	end
	...
	item_button_fs(fs, 4, 1, recipe.output, recipe.output:match("%S*"), nil, 1.05)
end
...
local function get_formspec(player)
	...
	if #data.items == 0 then
		...
		for i = first_item, first_item + 31 do
			...
			item_button_fs(fs, x, y, item, item, nil, 1.05)
		end
	end
