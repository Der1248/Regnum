...
minetest.register_chatcommand("/setpos1", {
	params = "<xt> <yt> <zt>",
	description = "Set WorldEdit region position 1 to the player's location",
	privs = {worldedit=true},
	func = function(name, param)
	local h = minetest.env:get_player_by_name(name):getpos()
	local found, _, xt, yt, zt = param:find("([^%s]+)%s+([^%s]+)%s+([^%s]+)")
		--pos.x = math.floor(xt)
		--pos.y = math.floor(yt)
		--pos.z = math.floor(zt)
		worldedit.pos1[name] = pos
		worldedit.mark_pos1(name)
		minetest.chat_send_player(name, "WorldEdit position 1 set to "..xt..","..yt..","..zt..","..tostring(minetest.env:get_player_by_name(name):getpos()).."")
	end,
})
minetest.register_chatcommand("/pos2", {
	...
	func = function(name, param)
		...
		worldedit.mark_pos2(name)
		minetest.chat_send_player(name, "WorldEdit position 2 set to (" .. pos.x .. "," .. pos.y .. "," .. pos.z .. ")"..tostring(pos).."")
	end,
})
